...
func init(pos):
	...
	position = pos;
	spawn_tween.interpolate_property(self, 'vel', Vector2(100, 20), Vector2(0, 300), 1, Tween.TRANS_LINEAR, Tween.EASE_IN);
	spawn_tween.start();
	STATE = 'SPAWN';
...
# warning-ignore:return_value_discarded
		...
		'MOVE':
			...
			if Input.is_action_pressed("player_left"):
				...
				vel = vel.linear_interpolate(Vector2(0, vel.y), delta*3);
				play('idle');
