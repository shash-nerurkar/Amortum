...
#stores the script
var code_script = [];
#[0] -> type of line #true->normal line, false->error line, int->function call, null->return call
#[1] -> line length
#[2] -> 1st object of the line
#[3] -> enemy type and class
var line_info := {}; 
...
func post_rotation():
	for obs in obstacle_container.get_children():
		if obs.type == null: 
			obs.type = 'DUMMY';
			obs.init();
		if obs.type == 'ENEMY' or obs.type == 'POWERUP' or obs.type == 'PORTAL':
			obs.init();
...
# warning-ignore:integer_division
	number_of_enemies = (max_enemies + randi() % 5)/2;
	populate_function();
...
func spawn_triggers():
	for j in range(0, func_endpoints.size(), 2): #iterate through all functions, including main
		var number_of_triggers = ceil((func_endpoints[j+1] - func_endpoints[j])/trigger_range); #find num of triggers required
		for i in range(number_of_triggers): 
			var t_pos = spawn_offset + tile_size*(func_endpoints[j] + trigger_range*i); #calculate position of each trigger
			var t = make_trigger('enemy', t_pos, 300 if rotate_to else 512);            #make a trigger at that position
			var end = min(func_endpoints[j] + trigger_range*(i+1), func_endpoints[j+1]);#end of range of the trigger
			for k in range(func_endpoints[j] + trigger_range*i, end):
				if line_info.keys().has(k):                         
					if line_info[k][0] == null:                    #if a return call exists, add to portal list    
						t.portal_list.append([line_info[k][0], k]);
					elif line_info[k][0] is int:                   #if a function call exists, add to portal list
						t.portal_list.append([line_info[k][0], k]);
					elif not line_info[k][0]:                      #if an error line exists, add to index list
...
func spawn_enemy(trigger_obj):
	...
	if trigger_obj.index_list.empty(): #if trigger has no enemies to spawn, then end fight
		check_container(true);
	else:
		kill_timer.start();
		#disable all triggers for the duration of the fight
		for t in trigger_container.get_children(): 
			if t.type == 'enemy':
				t.detector.set_deferred('disabled', true);
		#iterate through trigger's enemy list and spawn enemies
		for i in trigger_obj.index_list: 
			#setting enemy spawn position and type
			var e_pos;
			for j in range(i, 0, -1):    #from the error line, go towards the left until a 
				if line_info[j][0]:      #normal line occurs (so enemy can spawn)
					e_pos = Vector2(spawn_offset + j*tile_size, 300); 
					break;
			var enemy_type_info = choose_enemy(e_pos);
			#making obstacles converge and setting charlist
			var start_index = obstacle_container.get_children().find(line_info[i][2]);#find the 1st obstacle of error line
			for j in range(code_script[i].length()):                                  #add characters of the obstacles to
				if obstacle_container.get_child(start_index + j).type == 'ENEMY':     #the charlist of the 1st obstacle,
					obstacle_container.get_child(start_index + j).converge(e_pos);    #and make them converge
					obstacle_container.get_child(start_index).charlist += obstacle_container.get_child(start_index + j).label.text;
			#after converge is done, init enemy
			...
			obstacle_container.get_child(start_index).connect('send_charlist', enemy_type_info[0], 'init', enemy_type_info[1]);

func choose_enemy(pos):
	var ground_enemy_attack_class_array := ['shoot', 'laser', 'melee'];
	var ground_enemy_attack_type_array := ['shotgun', 'pulse', 'sniper'];
	var fly_enemy_attack_class_array := ['shoot', 'laser', 'melee'];
	var fly_enemy_attack_type_array := ['homing', 'pulse', 'bomber', 'dash_bomber'];
	var e = randi() % 2;
	var enemy_type_info = [null];
	match(e):
		0:
			e = ground_enemy.instance();
			enemy_type_info.append([ground_enemy_attack_class_array[randi() % ground_enemy_attack_class_array.size()],
									ground_enemy_attack_type_array[randi() % ground_enemy_attack_type_array.size()]]);
		1:
			e = fly_enemy.instance();
			enemy_type_info.append([fly_enemy_attack_class_array[randi() % fly_enemy_attack_class_array.size()],
								   fly_enemy_attack_type_array[randi() % fly_enemy_attack_type_array.size()]]);
	e.player = player;
	e.position = pos;
...
func check_container(force_check := false):
	...
	if count > 1:
		kill_timer.start();
	elif count == 1 or force_check:
		on_trigger_finish();

func on_trigger_finish():
	for t in range(current_trigger.portal_list.size()):
		var portal_pos = Vector2(spawn_offset + tile_size*current_trigger.portal_list[t][1], 300) if rot_state else Vector2(512, spawn_offset + tile_size*current_trigger.portal_list[t][1]);
		var p = create_portal(true, current_func, portal_pos, current_trigger.portal_list[t][0]);
		var i = current_trigger.portal_list[t][1];
		var start_index = obstacle_container.get_children().find(line_info[i][2]);
		for j in range(code_script[i].length()):
			if obstacle_container.get_child(start_index + j).type == 'PORTAL':
				obstacle_container.get_child(start_index + j).converge(portal_pos);
		obstacle_container.get_child(start_index).pos_tween.connect('tween_all_completed', obstacle_container.get_child(start_index), 'convergence_complete');
		obstacle_container.get_child(start_index).add_user_signal('spawn_portal');
		obstacle_container.get_child(start_index).connect('spawn_portal', p, 'spawn');
	current_trigger.queue_free();
	for t in trigger_container.get_children():
		t.detector.set_deferred('disabled', false);

func _on_kill_timer_timeout():
	var e = enemy_container.get_child(0);
		...
		if number_of_lines - extra_lines - i > powerup_types[powerup_list[i]].size(): #checking availability of length
			for j in range(i, i + powerup_types[powerup_list[i]].size()):
				if line_info[j][1] < powerup_types[powerup_list[i]][j-i]:
					...
					line_info[j][1] = powerup_types[powerup_list[i]][j-i];
		else:
			break; 
		...
		if number_of_lines - extra_lines - i > platform_types[platform_list[i]].size(): #checking availability of length
			for j in range(i, i + platform_types[platform_list[i]].size()):
				if line_info[j][1] < platform_types[platform_list[i]][j-i]:
					code_script[j] = '\treplaced(n);';#replacing code
					line_info[j][1] = platform_types[platform_list[i]][j-i];
		else:
			break; 
...
func level_cleared():
	level += 1;
	max_enemies += 5;
	number_of_enemies = max_enemies + randi()%5; 
	HUD.transition.show();
